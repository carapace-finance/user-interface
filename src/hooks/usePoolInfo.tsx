  ...
  const updatePool = async () => {
    ...
    if (isAuthenticated && isWeb3Enabled && web3?.currentProvider) {
      try {
        ...
        const _loanDefaulted = await loanContract.methods
          .loanDefaulted()
          .call();
        ...
        setTotalPremium(
          _premium == 0
            ? "0"
            : `${(+formatUnits(_premium, 18)).toFixed(3).replace(/\.000$/, "")}`
        );
        setTotalCoverage(
          _coverage == 0
            ? "0"
            : `${(+formatUnits(_coverage, 18))
                .toFixed(3)
                .replace(/\.000$/, "")}`
        );
