...
export const AccountProvider = ({ children }) => {
  const { web3, isWeb3Enabled, enableWeb3, user, isAuthenticated } =
    useMoralis();
  ...
  const updateBalances = async () => {
    const daiTokenContract = new web3.eth.Contract(
      daiToken,
      DAI_TOKEN_ADDRESS_DEV
    );
    const coverTokenContract = new web3.eth.Contract(
      coverToken,
      COVER_TOKEN_ADDRESS_DEV
    );
    const premTokenContract = new web3.eth.Contract(
      premToken,
      PREM_TOKEN_ADDRESS_DEV
    );

    // const daiBalance = await BalanceHelper.getERC20TokenBalance('DAI');
    const daiBalance = await daiTokenContract?.methods
      .balanceOf(user.get("ethAddress"))
      .call();
    const premBalance = await premTokenContract?.methods
      .balanceOf(user.get("ethAddress"))
      .call();
    const coverBalance = await coverTokenContract?.methods
      .balanceOf(user.get("ethAddress"))
      .call();
    const premTotalSupply = await premTokenContract?.methods
      .totalSupply()
      .call();
    const coverTotalSupply = await coverTokenContract?.methods
      .totalSupply()
      .call();
    ...
    _setDaiBalance(
      daiBalance == 0
        ? "0"
        : `${(+formatUnits(daiBalance, 18)).toFixed(3).replace(/\.000$/, "")}`
    );
    _setPremBalance(
      premBalance == 0
        ? "0"
        : `${(+formatUnits(premBalance, 18)).toFixed(3).replace(/\.000$/, "")}`
    );
    _setCoverBalance(
      coverBalance == 0
        ? "0"
        : `${(+formatUnits(coverBalance, 18)).toFixed(3).replace(/\.000$/, "")}`
    );
  ...
  const approveDai = async (poolAddress: string, amount: number) => {
    ...
    await daiTokenContract.methods
      .approve(poolAddress, convertedAmount)
      .send({ from: user.get("ethAddress") })
      .then((res) => console.log(res));
    const allowance = await daiTokenContract.methods
      .allowance(user.get("ethAddress"), poolAddress)
      .call();
  ...
  const approveCover = async (poolAddress: string, amount: number) => {
    ...
    const amountToApprove = web3.utils.toWei(amount.toString(), "ether");
    const convertedAmount = web3.utils.toBN(amountToApprove);
    await coverTokenContract.methods
      .approve(poolAddress, convertedAmount)
      .send({ from: user.get("ethAddress") })
      .then((res) => console.log(res));
    const allowance = await coverTokenContract.methods
      .allowance(user.get("ethAddress"), poolAddress)
      .call();
    return allowance === amountToApprove;
  };
  ...
  const approvePrem = async (poolAddress: string, amount: number) => {
    ...
    const amountToApprove = web3.utils.toWei(amount.toString(), "ether");
    const convertedAmount = web3.utils.toBN(amountToApprove);
    await premTokenContract.methods
      .approve(poolAddress, convertedAmount)
      .send({ from: user.get("ethAddress") })
      .then((res) => console.log(res));
    const allowance = await premTokenContract.methods
      .allowance(user.get("ethAddress"), poolAddress)
      .call();
    return allowance === amountToApprove;
  };
